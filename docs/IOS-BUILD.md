...
The build signs with a certificate bound to a private key that you generate
once and store in Codemagic as a secure variable. Same key every build, so the
certificate is created on the first run and reused after that.

Generate the key. On a Mac, in Terminal:

```bash
openssl genrsa 2048 | pbcopy
```

On Windows, in PowerShell (Git for Windows ships OpenSSL):

```powershell
& "C:\Program Files\Git\usr\bin\openssl.exe" genrsa 2048 | Set-Clipboard
```

The key is now on your clipboard. Then in Codemagic: **Applications**,
Teraweights, **Environment variables** tab. Variable name
`CERTIFICATE_PRIVATE_KEY`, paste the key as the value, group `signing`, tick
**Secure**, click **Add**.

Why this and not Codemagic's own signing: your Apple team already has
distribution certificates from other apps, but Codemagic holds none of their
private keys, so it can sign with none of them, and its automatic mode only
matches provisioning profiles it already has rather than creating one.
...
1. `CERTIFICATE_PRIVATE_KEY` exists as a secure variable in group `signing` on the Teraweights app, with the full PEM text including the BEGIN and END lines.
2. The API key has **App Manager** access. Developer access cannot create certificates.
3. Your team has fewer than three distribution certificates. Apple caps it; revoke an unused one at developer.apple.com, Certificates, or delete the unused `teraweights_dist` one in Codemagic, Code signing identities.
