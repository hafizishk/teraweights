...
## The provisioning profile is created by the build, not by Codemagic

- With the generated certificate in place, `ios_signing: {distribution_type, bundle_identifier}` still failed with "No matching profiles found": that mode only matches profiles already stored in Code signing identities and never creates one.
- The build now creates the App Store profile itself through the App Store Connect API: look up (or register) the bundle ID, list every unexpired distribution certificate on the team, delete any previous "Teraweights App Store" profile, create a fresh one carrying all of those certificates, save it to disk and install it in both Xcode profile folders. Creating a profile needs only certificate IDs, never a private key, which is why this succeeds where fetching did not.
- The certificate itself comes in through `ios_signing.certificates: [teraweights_dist]`, which imports Codemagic's generated certificate and key into the keychain before scripts run. `xcodebuild` then gets team, profile UUID and identity explicitly.

## Signing key is a secure variable
...
## Confirmation sheet and Account hub

- Booking a class, joining a waitlist, booking a PT slot and registering for an event all end on the same `BookedSheet`: a tick, what was booked, when and where, then "Add to calendar" and Done. Before, the answer was a toast that vanished in three seconds. Cancelling stays a toast; there is nothing to confirm about a cancellation.
- "Add to calendar" is an `.ics` from `/api/calendar` on Apple devices (iOS opens it straight into Calendar; the shell does the same) and a Google Calendar link everywhere else, including the Android shell. The route re-reads the session, PT session or event under the member's own session so nobody can pull another member's booking. Events without a chosen slot go in as an all-day entry. The `.ics` builder in `lib/rules/calendar.ts` is pure and unit-tested (CRLF, escaping, line folding).
- Account is a hub, the way a settings screen is: the member's photo and name at the top, then rows. The editable form moved one tap in to Your details. Packages and Personal training rows link to the screens that already exist. Support and Legal are new pages. Support's WhatsApp and email come from `NEXT_PUBLIC_SUPPORT_WHATSAPP` and `NEXT_PUBLIC_SUPPORT_EMAIL`; a row is hidden until its value is set so nothing invented ships. Legal is house rules and a plain-language data note written from what the app enforces, marked as a draft until Teraweights confirm the wording.

## Light mode: a profile setting, dark by default

- `profiles.theme` is `dark`, `light` or `system`, picked under Appearance on Account. It lives on the profile, not the device, so it follows the member between the browser, the installed PWA and the native shell, and there is no `localStorage`.
- The member layout sets `data-theme` on its root and the CSS swaps the ink and paper tokens underneath it: ink becomes the warm off-white, paper the near-black, muted and PRIME darken a step for contrast. Because every component uses those tokens, nothing else changed. Admin and the public event pages stay dark. `system` follows `prefers-color-scheme`, and the status bar colour follows the choice through `generateViewport`.
- New `--color-on-brand` is the off-white for type on a red or blue fill, whatever the theme; buttons, East and West badges, the error toast and the segmented control use it instead of `text-paper`.
- Light palette is "paper" (`#f4f1ec` page, `#fbfaf7` surfaces, `#ddd7cd` hairlines): the brief's off-white promoted to the page. Three other palettes (white, stone, blush) were mocked up and can be swapped in by editing seven values in `globals.css`.
- Open: duotone photo heroes fade into the page colour, which reads washed on a light page. If light mode is kept, the heroes should stay dark in both modes.

## Check scripts follow the calendar

- The seed is anchored to the demo week, but `book_pt_session` and `apply_booking` check the real clock, so `npm run db:check` began failing on 16 Sep with "That time has passed" and "Booking closed". `db-assert.sql` now books PT on the next Thursday from today; `db-booking-test.sql` slides sessions and package dates forward by whole weeks inside its rolled-back transaction, so weekdays and every demo step hold. The seed itself is untouched.

## Connected health, part one: the web side

- Agreed 18 Sep 2026 to put wearable data in the demo. Two parts: the screens and data model first (this), the native plugin that reads Apple Health and Health Connect second. On iPhone nearly every wearable writes to Apple Health (Whoop, Amazfit through Zepp, Garmin, Apple Watch); Android has Health Connect. One plugin covers all of them, so there is no Whoop or Amazfit integration of our own.
- `session_metrics` holds one row per attended booking: average and max heart rate, calories, and the samples as `[[seconds, bpm], ...]`. `health_workouts` holds the member's own runs and rides. Both are the member's alone: RLS gives no coach, admin or other member a read, and the Legal page says so. `profiles.health_source`, `health_device` and `max_hr` record the connection and an optional real max.
- Zones are computed from samples at read time in `lib/rules/zones.ts`: five bands at 60, 70, 80 and 90% of max, each sample covering the gap to the next. Profiles carry no date of birth, so the max is 190 until the member sets their own. Comparison and the one-line sentence are rules too, unit-tested.
- Screens: the Your session card on You (last session with data, taps to `/app/session/[id]`), the session page with the heart-rate trace, zones, numbers and a comparison against the last session of the same class, This week on You (red bars for sessions, blue for own workouts, shown only once a store is connected), and Connected health under Account. Own workouts count toward the week and never the streak; the streak rule is untouched.
- Seed: Aisyah has Apple Health connected through a Whoop, her fourteen attended sessions carry generated traces (one sample a minute, a hard middle, rising a little each fortnight) and she has two 5k runs, 11 and 16 Sep. Names, dates and times of the demo are unchanged.
- The connect button is inert on the web and says so. Part two adds the Capacitor plugin, the HealthKit capability on the App ID, the usage strings, the sync after check-in, and new TestFlight and Android builds. Nothing here reads a device yet.

## Three demo profiles: before, during, after

- Agreed 18 Sep 2026. The seed's member stories change so one demo can show every member stage. **Marcus** has nothing: not onboarded, no package, never used the free week, so he demos first sign-in, the empty state and starting the trial live. **Priya** is on the free Trial Week, day 5 of 7 (14–21 Sep), with one East session attended and one booked; her expired 1-month stays as history. **Aisyah** is the full member: the Weekday 4-month ending 28 Sep, credits, and now the PT 8-pack with 3 left and Faizal as coach, which moved from Marcus with the same dates and notes plus one more attended session on 8 Sep.
- This edits data the brief's section 11 describes (Marcus's PRO and PT, Priya's empty state). Nothing about Aisyah's classes, the schedule, events or results changes, and the demo script's dates hold. The check scripts follow the new stories.
- PT metrics: `session_metrics` now points at a class booking or a PT session, one or the other. PT home gets a "Your last PT" card and a zone strip on each past session, `/app/pt/[id]` is the PT session page with the coach's note first, and This week shows PT in PRIME yellow. PT attended sessions count toward the week and the streak, the same as a class: the streak rule is unchanged, its input now includes them.
- `scripts/health-demo.sql` is gone: the hosted database is reseeded from `supabase/seed.sql`, which carries the health data.
