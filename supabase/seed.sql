...
-- ---------------------------------------------------------------------------
-- Connected health (scope change). Aisyah has Apple Health connected through
-- a Whoop, so her 14 attended sessions carry a heart-rate trace and the week
-- view has her own runs. Traces are generated, one sample a minute, rising to
-- a peak in the middle of the hour; the numbers are consistent with the rule
-- in lib/rules/zones.ts (max 190 by default) and the demo script.
-- ---------------------------------------------------------------------------
update public.profiles
...
begin
  ...
  loop
    i := i + 1;
    -- Fitter every fortnight: the same session sits a little higher each time.
    base := 98 + (i % 3) * 2;
    peak := 158 + i * 0.9;
    samples := '[]'::jsonb;
    total := 0;
    mx := 0;
    for t in 0..60 loop
      -- Warm-up, a long hard middle, a fade at the end, plus a little wobble.
