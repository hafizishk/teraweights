...
- **Community feed.** Replaces the Telegram channel. `announcements` is the posts table: `slug`, `category` (announcement, news, recipe, photos), `cover_url`, `images[]`, `archived_at`, plus the existing audience targeting. One-way: coaches post, members read, no comments. `/app/community` is the Feed tab; the newest post is Home's card. Tabs are Book, Packs, You (raised circle with the member's photo, the landing screen at `/app`: next session, streak, stage nudge, pulse, latest post, My PA.ROX, coaches, PT, packages), Feed, Events; the header avatar (top right, beside the credits word) opens `/app/account`, a hub: Your details (photo, name, phone, zone), Packages, Personal training, Support (contacts from `NEXT_PUBLIC_SUPPORT_EMAIL` / `NEXT_PUBLIC_SUPPORT_WHATSAPP`), Legal, install, sign out. There is no separate Home. Every booking, waitlist join, PT booking and event registration ends on `BookedSheet` with "Add to calendar" (`.ics` from `/api/calendar` on Apple, Google Calendar link elsewhere; builder in `lib/rules/calendar.ts`). Admin composer at `/admin/announcements` uploads to the `posts` bucket. Body is a small markdown subset parsed by `lib/rules/post-body.ts`.
...
- **Connected health.** Apple Health and Health Connect, read-only, opt-in, native app only. `session_metrics` (heart-rate samples per attended booking) and `health_workouts` (own runs and rides), both member-only under RLS; `profiles.health_source`, `health_device`, `max_hr`. Zones, comparison and copy in `lib/rules/zones.ts`. Screens: Your session card on You, `/app/session/[id]`, This week on You, `/app/account/health`. Own workouts never count toward the streak. PT sessions carry the same metrics (`pt_session_id`), shown on PT home and `/app/pt/[id]`. Part two (Capacitor plugin, HealthKit capability, sync after check-in) is not built yet.
...
## Seed data is sacred
`supabase/seed.sql` implements brief section 11, with the three demo profiles agreed 18 Sep 2026 (Marcus before: nothing; Priya during: free week; Aisyah after: membership, credits, PT with Faizal, Apple Health). Demo member is Aisyah Rahman. Do not invent different names, dates, or times — the demo script depends on them. `npm run db:reset` must reset and reseed cleanly.
